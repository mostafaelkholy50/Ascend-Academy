<x-dashboard-layout title="Edit Profile">
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <a href="{{ route('parent.profile.show') }}" class="bg-white hover:bg-gray-50 p-3 rounded-xl shadow-md hover:shadow-lg transition-all duration-200 border border-gray-200">
    ...
    <div class="max-w-3xl space-y-6">
        ...
        <div class="bg-white rounded-3xl shadow-lg p-8 border border-gray-100">
            ...
            <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left">
                ...
                <div class="flex-1">
                    <form action="{{ route('parent.profile.avatar.update') }}" method="POST" enctype="multipart/form-data" class="space-y-3">
                        @csrf
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Choose new profile picture</label>
                            <input type="file" name="avatar" accept="image/jpeg,image/png,image/jpg,image/gif" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition">
                    ...
                    @if($user->avatar)
                        <form action="{{ route('parent.profile.avatar.delete') }}" method="POST" class="mt-2" onsubmit="return confirm('Are you sure you want to remove your profile picture?');">
                            @csrf
                            @method('DELETE')
        ...
        <form action="{{ route('parent.profile.update') }}" method="POST" class="bg-white rounded-3xl shadow-lg p-8 border border-gray-100">
            @csrf
            @method('PATCH')
            ...
            <div class="flex flex-col sm:flex-row gap-4 mt-8">
                ...
                <a href="{{ route('parent.profile.show') }}" class="w-full sm:w-auto text-center px-6 py-4 border-2 border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 transition font-bold">
                    Cancel
                </a>
            </div>
        </form>
