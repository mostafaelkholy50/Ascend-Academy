<x-dashboard-layout title="Daily Schedule">
    ...
    <div class="mb-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">Daily Schedule</h1>
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <form method="GET" class="flex items-center gap-2">
                    ...
                    <select name="child_id" onchange="this.form.submit()" class="px-4 py-2 bg-white border border-gray-300 rounded-xl text-sm font-medium text-gray-700 focus:ring-2 focus:ring-green-500 focus:border-green-500">
        ...
        <div class="bg-white rounded-3xl shadow-lg overflow-hidden border border-gray-100">
            <div class="divide-y divide-gray-100">
                @foreach($schedules as $schedule)
                    <div class="p-6 hover:bg-gradient-to-r hover:from-green-50 hover:to-emerald-50 transition-all duration-200">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-4 flex-grow">
                                <div class="w-16 h-16 bg-gradient-to-br from-green-400 to-emerald-500 rounded-2xl flex items-center justify-center shadow-lg text-white">
                                    <i class="fa-solid fa-book-quran text-2xl"></i>
                                </div>
                                <div class="flex-grow">
                                    <div class="flex items-center gap-2 mb-2">
                                        <h4 class="font-bold text-gray-800 text-lg">{{ $schedule->course->title ?? 'Session' }}</h4>
                                        <span class="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-lg text-sm font-bold">
                                            {{ $schedule->student->name }}
                                        </span>
                                    </div>
                                    <div class="flex flex-wrap gap-4 text-sm text-gray-600">
                                        <span><i class="fa-solid fa-user-tie mr-2 text-green-600"></i>{{ $schedule->teacher->name }}</span>
                                        <span><i class="fa-solid fa-clock mr-2 text-green-600"></i>{{ $schedule->starts_at->format('g:i A') }} - {{ $schedule->ends_at->format('g:i A') }}</span>
                                        <span><i class="fa-solid fa-hourglass-half mr-2 text-green-600"></i>{{ $schedule->duration }} minutes</span>
                                    </div>
                                    @if($schedule->zoom_link)
                                        <div class="mt-2">
                                            <a href="{{ $schedule->zoom_link }}" target="_blank" class="text-sm text-blue-600 hover:text-blue-700 font-medium">
                            ...
                            <div class="flex flex-col items-end gap-2">
                                ...
                                @if($schedule->starts_at->isFuture())
                                    ...
                                    @if($minutesUntil < 60)
                                        <span class="px-3 py-1 bg-gradient-to-r from-green-100 to-emerald-100 text-green-700 rounded-lg text-xs font-bold">
                                            <i class="fa-solid fa-clock mr-1"></i>In {{ $minutesUntil }} min
                                        </span>
                                    @endif
                                @elseif($schedule->starts_at->isPast() && $schedule->ends_at->isFuture())
                                    <span class="px-3 py-1 bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-700 rounded-lg text-xs font-bold animate-pulse">
                            ...
                            <div class="mt-4 pt-4 border-t border-gray-200">
                                <div class="flex items-center gap-4 text-sm">
                                    ...
                                    <span class="flex items-center gap-2">
                                        ...
                                        <span class="px-2 py-1 rounded-lg text-xs font-bold {{ $attendance->student_present ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                    ...
                                    <span class="flex items-center gap-2">
                                        ...
                                        <span class="px-2 py-1 rounded-lg text-xs font-bold {{ $attendance->teacher_present ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
