            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <form method="GET" class="flex items-center gap-2">
                    <select name="child_id" onchange="this.form.submit()"
                        class="px-4 py-2 bg-white border border-gray-300 rounded-xl text-sm font-medium text-gray-700 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-4">
        @foreach ($schedulesByDay as $dayKey => $dayData)
            ...
            <div class="bg-white rounded-2xl shadow-md overflow-hidden {{ $isToday ? 'ring-2 ring-indigo-500' : '' }}">
                <!-- Day Header -->
                <div
                    class="p-4 {{ $isToday ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white' : 'bg-gray-50' }} border-b">
                ...
                <div class="p-3 space-y-2 min-h-[200px]">
                    @if ($schedules->count() > 0)
                        @foreach ($schedules as $schedule)
                            ...
                            <div
                                class="p-3 bg-{{ $color }}-50 border border-{{ $color }}-100 rounded-xl hover:shadow-md transition">
