<x-dashboard-layout title="Student Attendance Profile - {{ $student->name }}">
    <!-- Page Header -->
    <div class="mb-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <a href="{{ route('admin.attendances.index') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium mb-2 inline-flex items-center">
                    <i class="fa-solid fa-arrow-left mr-1"></i> Back to Attendances
                </a>
                <h1 class="text-3xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">{{ $student->name }}</h1>
                <p class="text-sm text-gray-500 mt-1"><i class="fa-solid fa-user-graduate mr-1"></i> Student Attendance Profile</p>
            </div>
        </div>
    </div>
    ...
    <!-- Teachers Summary -->
    <div class="mb-8">
        <h2 class="text-xl font-bold text-gray-800 mb-4">Teachers Summary</h2>
            ...
            @forelse($teachers as $teacher)
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            <h3 class="font-bold text-gray-900 text-sm">{{ $teacher->name }}</h3>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div class="bg-gray-50 rounded-xl p-3 text-center">
                            ...
                            <p class="text-lg font-black text-gray-800">{{ $teacher->total_sessions }}</p>
                        </div>
                        <div class="bg-green-50 rounded-xl p-3 text-center">
                            <p class="text-[10px] text-green-600 uppercase font-bold">Attended</p>
                            <p class="text-lg font-black text-green-700">{{ $teacher->attended_count }}</p>
                        </div>
                        <div class="bg-red-50 rounded-xl p-3 text-center">
                            <p class="text-[10px] text-red-600 uppercase font-bold">Absent</p>
                            <p class="text-lg font-black text-red-700">{{ $teacher->absent_count }}</p>
                        </div>
                        <div class="bg-orange-50 rounded-xl p-3 text-center">
                            <p class="text-[10px] text-orange-600 uppercase font-bold">Tch Abs</p>
                            <p class="text-lg font-black text-orange-700">{{ $teacher->teacher_absent_count }}</p>
                        </div>
                    </div>
                </div>
            @empty
                <div class="col-span-full bg-gray-50 rounded-3xl p-8 text-center border border-gray-100">
                    <p class="text-gray-500">No teachers found for this student.</p>
                </div>
            @endforelse
        </div>
    </div>
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Teacher</th>
                    ...
                    @forelse($attendances as $attendance)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 max-w-xs">
                                @if(!$attendance->student_present || !$attendance->teacher_present || $attendance->remark || $attendance->student_report)
                                    <div class="text-xs text-gray-600 line-clamp-2" title="{{ $attendance->remark ?? $attendance->student_report ?? 'No reason provided' }}">
                                        {{ $attendance->remark ?? $attendance->student_report ?? 'No reason provided' }}
                                    </div>
                                @else
            ...
            @forelse($attendances as $attendance)
                <div class="p-5 hover:bg-gray-50 transition">
                    ...
                    <div class="mb-3 text-sm">
                        <span class="text-gray-500">Teacher:</span> <span class="font-bold text-gray-800">{{ $attendance->schedule->teacher->name }}</span>
                    </div>
                    ...
                    @if(!$attendance->student_present || !$attendance->teacher_present || $attendance->remark || $attendance->student_report)
                        <div class="bg-yellow-50 border border-yellow-100 rounded-lg p-3 mt-3">
                            ...
                            <p class="text-xs text-yellow-900">{{ $attendance->remark ?? $attendance->student_report ?? 'No reason provided' }}</p>
                        </div>
                    @endif
                </div>
            @empty
