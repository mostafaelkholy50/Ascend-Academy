<x-dashboard-layout title="Edit Profile">
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <a href="{{ route('admin.profile.show') }}" class="bg-white hover:bg-gray-50 p-3 rounded-xl shadow-md hover:shadow-lg transition-all duration-200 border border-gray-200">
    ...
    <div class="max-w-3xl space-y-6">
        ...
        <div class="bg-white rounded-3xl shadow-lg p-8 border border-gray-100">
            ...
            <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left">
                ...
                <div class="flex-1">
                    <form action="{{ route('admin.profile.avatar.update') }}" method="POST" enctype="multipart/form-data" class="space-y-3">
                        @csrf
                        <div>
                    ...
                    @if($user->avatar)
                        <form action="{{ route('admin.profile.avatar.delete') }}" method="POST" class="mt-2" onsubmit="return confirm('Are you sure you want to remove your profile picture?');">
                            @csrf
                            @method('DELETE')
        ...
        <form action="{{ route('admin.profile.update') }}" method="POST" class="bg-white rounded-3xl shadow-lg p-8 border border-gray-100">
            @csrf
            @method('PATCH')
            ...
            <div class="flex flex-col sm:flex-row gap-4 mt-8">
                ...
                <a href="{{ route('admin.profile.show') }}" class="w-full sm:w-auto text-center px-6 py-4 border-2 border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 transition font-bold">
                    Cancel
                </a>
            </div>
        </form>
