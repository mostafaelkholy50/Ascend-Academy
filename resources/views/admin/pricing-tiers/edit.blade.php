<x-dashboard-layout title="Edit Pricing Tier">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800">Edit Pricing Tier</h1>
        <p class="text-gray-600 text-sm">Update pricing tier information</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.pricing-tiers.update', $pricingTier->id) }}" class="max-w-4xl">
        @csrf
        @method('PATCH')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="days_per_week" id="days_per_week" required
                        ...
                        @for($i = 1; $i <= 7; $i++)
                            <option value="{{ $i }}" {{ old('days_per_week', $pricingTier->days_per_week) == $i ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="session_duration" id="session_duration" required
                        ...
                        <option value="30" {{ old('session_duration', $pricingTier->session_duration) == '30' ? 'selected' : '' }}>30 minutes</option>
                        <option value="60" {{ old('session_duration', $pricingTier->session_duration) == '60' ? 'selected' : '' }}>60 minutes (1 hour)</option>
                    </select>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="price_cad" id="price_cad" step="0.01" min="0" 
                            value="{{ old('price_cad', $pricingTier->price_cad) }}" required
                            placeholder="0.00"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="price_usd" id="price_usd" step="0.01" min="0" 
                            value="{{ old('price_usd', $pricingTier->price_usd) }}" required
                            placeholder="0.00"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="price_gbp" id="price_gbp" step="0.01" min="0" 
                            value="{{ old('price_gbp', $pricingTier->price_gbp) }}" required
                            placeholder="0.00"
                            class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-vibrant-green focus:border-transparent">
                    </div>
                    @error('price_gbp')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        ...
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-vibrant-green focus:border-transparent">{{ old('notes', $pricingTier->notes) }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="flex gap-4">
            <button type="submit" class="bg-vibrant-green text-white px-8 py-3 rounded-lg hover:bg-deep-blue transition font-semibold">
                <i class="fa-solid fa-save mr-2"></i>Update Pricing Tier
            </button>
