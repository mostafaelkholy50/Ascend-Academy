<x-dashboard-layout title="Edit Course">
    <div class="mb-6">
        <a href="{{ route('admin.courses.show', $course->id) }}" class="text-vibrant-green hover:underline text-sm">
            <i class="fa-solid fa-arrow-left mr-2"></i>Back to Course Details
        </a>
    </div>
        ...
        <form action="{{ route('admin.courses.update', $course->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="5" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-vibrant-green @error('description') border-red-500 @enderror">{{ old('description', $course->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="flex items-center space-x-4 mt-3">
                    <label class="flex items-center cursor-pointer">
                        <input type="radio" name="is_free" value="0" {{ old('is_free', $course->is_free ? '1' : '0') == '0' ? 'checked' : '' }}
                            class="mr-2 text-vibrant-green focus:ring-vibrant-green">
                        <span class="text-sm text-gray-700">Paid</span>
                    </label>
                    <label class="flex items-center cursor-pointer">
                        <input type="radio" name="is_free" value="1" {{ old('is_free', $course->is_free ? '1' : '0') == '1' ? 'checked' : '' }}
                            class="mr-2 text-vibrant-green focus:ring-vibrant-green">
            ...
            <div>
                ...
                @if($course->photo)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $course->photo) }}" alt="Current Photo" class="h-32 w-auto rounded-lg object-cover">
                ...
                <p class="text-xs text-gray-500 mt-1">Recommended size: 800x600px. Max 2MB. Leave empty to keep current photo.</p>
                @error('photo')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex gap-4">
                ...
                <a href="{{ route('admin.courses.show', $course->id) }}" class="bg-gray-300 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-400 transition">
                    Cancel
                </a>
            </div>
        </form>
