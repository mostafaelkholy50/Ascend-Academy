<x-dashboard-layout>
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-deep-blue">Edit Book: {{ $book->title }}</h1>
            <p class="text-sm text-gray-500 mt-1">Update book details or replace the PDF file and book cover</p>
        </div>
    ...
    <div class="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 max-w-3xl mx-auto">
        <form id="book-form" action="{{ route('books.update', $book) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PATCH')
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $book->title) }}" required placeholder="Enter textbook title..." 
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" id="description" rows="4" placeholder="Write a short description of the textbook contents..." 
                    class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-vibrant-green focus:border-transparent transition-all">{{ old('description', $book->description) }}</textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- PDF File -->
                <div class="space-y-2">
                    <label for="pdf_file" class="block text-sm font-semibold text-gray-700">Update Book File (Optional)</label>
                    <div class="relative border-2 border-dashed border-gray-200 rounded-xl p-4 hover:border-vibrant-green transition cursor-pointer bg-gray-50 flex flex-col items-center justify-center text-center">
                        <input type="file" name="pdf_file" id="pdf_file" accept="application/pdf" class="absolute inset-0 opacity-0 w-full h-full cursor-pointer">
                        <i class="fa-solid fa-file-pdf text-3xl text-gray-400 mb-2"></i>
                        <span class="text-xs font-semibold text-gray-600 block">Choose a new PDF file to replace current</span>
                        <span class="text-[10px] text-gray-400 mt-1 block">Leave empty to keep current file</span>
                    </div>
                    <div class="text-xs text-gray-500 font-medium flex items-center gap-1.5 mt-1.5">
                        <i class="fa-solid fa-file-circle-check text-vibrant-green"></i>
                        Current file: <span class="text-gray-700 select-all font-mono">{{ basename($book->file_path) }}</span>
                    </div>
                ...
                <!-- Cover Image -->
                <div class="space-y-2">
                    <label for="cover_image" class="block text-sm font-semibold text-gray-700">Update Cover Image (Optional)</label>
                    <div class="relative border-2 border-dashed border-gray-200 rounded-xl p-4 hover:border-vibrant-green transition cursor-pointer bg-gray-50 flex flex-col items-center justify-center text-center">
                        ...
                        <span class="text-xs font-semibold text-gray-600 block">Choose a new cover image</span>
                        <span class="text-[10px] text-gray-400 mt-1 block">Leave empty to keep current cover</span>
                    </div>
                    @if($book->cover_image)
                        <div class="text-xs text-gray-500 font-medium flex items-center gap-1.5 mt-1.5">
                            <i class="fa-solid fa-image text-vibrant-green"></i>
                            A custom cover image currently exists
                        </div>
                    @endif
            ...
            <div class="flex items-center gap-3 bg-gray-50 p-4 rounded-xl border border-gray-100">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $book->is_active) ? 'checked' : '' }} 
    ...
    <div id="upload-progress-modal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-slate-900/60 backdrop-blur-sm transition-all duration-300">
        <div class="bg-white rounded-3xl p-8 max-w-md w-full mx-4 shadow-2xl border border-gray-100 transform scale-95 transition-transform duration-300">
            <div class="flex flex-col items-center text-center">
                ...
                <h3 class="text-xl font-bold text-deep-blue mb-1" id="upload-status-title">Updating Book...</h3>
        ...
        <script>
            ...
            document.getElementById('pdf_file').addEventListener('change', function(e) {
                ...
                if (e.target.files.length > 0) {
                    fileNameDiv.textContent = 'New file selected: ' + e.target.files[0].name;
            ...
            document.getElementById('cover_image').addEventListener('change', function(e) {
                ...
                if (e.target.files.length > 0) {
                    fileNameDiv.textContent = 'New image selected: ' + e.target.files[0].name;
                    fileNameDiv.classList.remove('hidden');
                } else {
                    fileNameDiv.classList.add('hidden');
                }
            ...
            document.getElementById('book-form').addEventListener('submit', function(e) {
                ...
                statusTitle.textContent = 'Updating Book...';
                ...
                // Set up XHR request (always POST, Laravel method spoofing field takes care of PATCH)
