...
<section class="bg-gradient-to-r from-vibrant-green to-teal-500 text-white rounded-3xl p-6 md:p-8 shadow-lg mb-6 relative overflow-hidden">
    <p class="text-sm opacity-90 mb-1">{{ now()->format('F j, Y') }}</p>
    <h2 class="text-2xl md:text-3xl font-bold mb-2">Welcome back, {{ $user->name }}!</h2>
    @if($message)
        <p class="opacity-95 text-sm">{{ $message }}</p>
    @else
        <p class="opacity-95 text-sm">Ready to continue your learning journey today.</p>
    @endif
