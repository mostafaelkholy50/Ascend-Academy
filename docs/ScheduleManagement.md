...
## Overview
The Schedule Management system in Ascend Academy handles the booking of classes for students with teachers. It supports both manual schedule creation (via the admin dashboard) and automated monthly schedule generation for active enrollments.
...
- **`App\Services\ScheduleService`**: Contains `storeSchedule()` and `generateMonthlySchedules()`. Both methods are fully transactional and rely on the model's conflict detection to abort and throw descriptive exceptions.
